...
const ContactForm: React.FC<ContactFormProps> = ({ data }) => {
  ...
  const title = data?.title || t.home.contactTitle
  const description =
    data?.description ||
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error || t.home.messageSendFailed)
      }

      setForm(initialState)
      setSuccess(t.home.messageSent)
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : t.home.messageSendFailed)
    } finally {
    ...
    <section className="w-full pt-[120px] pb-[80px] px-5 flex flex-col items-start max-w-[1280px] mx-auto overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row justify-center items-start gap-[64px] self-stretch">
          ...
          <div className="flex flex-col items-start gap-[16px] self-stretch">
            ...
            <div className="flex flex-col md:flex-row items-center gap-[16px] self-stretch">
              ...
              <div className="flex p-[16px] flex-col items-start gap-[16px] flex-1 self-stretch rounded-[20px] bg-transparent border border-transparent">
                <div className="flex items-center gap-[16px] self-stretch">
                  ...
                  <span className="text-[#EEE] font-poppins text-[16px] font-normal leading-[26px]">{t.home.phoneLabel}</span>
                </div>
              ...
              <div className="flex p-[16px] flex-col items-start gap-[16px] flex-1 self-stretch rounded-[20px] bg-transparent border border-transparent">
                <div className="flex items-center gap-[16px] self-stretch">
                  ...
                  <span className="text-[#EEE] font-poppins text-[16px] font-normal leading-[26px]">{t.home.emailLabel}</span>
                </div>
                <span className="self-stretch text-[#FCFCFC] font-poppins text-[20px] font-medium leading-[32px]">
                  {contactEmail}
                </span>
            ...
            <div className="flex p-[16px] flex-col items-start gap-[16px] self-stretch rounded-[20px] bg-transparent border border-transparent">
              <div className="flex items-center gap-[16px] self-stretch">
                ...
                <span className="text-[#EEE] font-poppins text-[16px] font-normal leading-[26px]">{t.home.addressLabel}</span>
              </div>
              <span className="self-stretch text-[#FCFCFC] font-poppins text-[20px] font-medium leading-[32px]">
                {address}
              </span>
        ...
        <div className="flex-1 lg:flex-none lg:w-[556px] flex flex-col p-8 py-10 items-center gap-[56px] self-stretch rounded-[20px] border border-[rgba(74,74,74,0.70)] bg-[#1A1A1A]">
          ...
          <form className="flex flex-col items-start gap-[24px] self-stretch" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col md:flex-row items-start gap-[24px] self-stretch">
              <div className="flex flex-col items-start gap-2 flex-1 self-stretch">
                <label className="text-[#EEE] font-poppins text-[16px] font-medium leading-[26px]">{t.home.firstName}</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col items-start gap-2 flex-1 self-stretch">
                <label className="text-[#EEE] font-poppins text-[16px] font-medium leading-[26px]">{t.home.lastName}</label>
                <input
                  type="text"
                  value={form.lastName}
            ...
            <div className="flex flex-col items-start gap-2 self-stretch">
              <label className="text-[#EEE] font-poppins text-[16px] font-medium leading-[26px]">{t.home.emailAddress}</label>
              <input
                type="email"
                value={form.email}
            ...
            {/* Subject */}
            <div className="flex flex-col items-start gap-2 self-stretch">
              <label className="text-[#EEE] font-poppins text-[16px] font-medium leading-[26px]">{t.home.subject}</label>
              <div className="relative self-stretch">
                <select
                  value={form.subject}
                  onChange={setField('subject')}
                  className="appearance-none flex p-[18px] justify-between items-center self-stretch rounded-[6px] border border-[rgba(74,74,74,0.70)] bg-[#0D0D0D] text-[#616161] font-poppins text-[14px] outline-none focus:border-[#F29F04] transition-all w-full cursor-pointer"
                >
                  <option value="">{t.home.subjectPlaceholder}</option>
                  <option value="General Inquiry">{t.home.subjectGeneralInquiry}</option>
            ...
            {/* Message */}
            <div className="flex flex-col items-start gap-2 self-stretch">
              <label className="text-[#EEE] font-poppins text-[16px] font-medium leading-[26px]">{t.home.message}</label>
              <textarea
                value={form.message}
            ...
            {error ? (
              <p className="self-stretch text-center font-poppins text-[14px] font-normal leading-[20px] text-[#FF9C9C]">
                {error}
              </p>
            ) : null}

            {success ? (
              <p className="self-stretch text-center font-poppins text-[14px] font-normal leading-[20px] text-[#06DF73]">
                {success}
              </p>
            ) : null}
            ...
            <button
              ...
              className="flex w-[180px] mt-8 py-[12px] px-[24px] mx-auto justify-center items-center gap-[12px] rounded-[80px] bg-[#F29F04] text-[#0D0D0D] font-poppins text-[16px] font-medium leading-[26px] transition-all hover:brightness-110 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isSubmitting ? t.common.sending : t.home.sendMessage}
            </button>
          </form>
        </div>
      </div>
