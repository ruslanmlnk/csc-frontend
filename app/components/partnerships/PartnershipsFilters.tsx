import React from 'react'
import MobileCategorySelector from '@/app/components/shared/MobileCategorySelector'

interface PartnershipsFiltersProps {
  searchValue: string
  onSearchChange: (value: string) => void
  onSearchSubmit: () => void
  locations: string[]
  verticals: string[]
  paymentModels: string[]
  locationValue: string
  verticalValue: string
  paymentModelValue: string
  onLocationChange: (value: string) => void
  onVerticalChange: (value: string) => void
  onPaymentModelChange: (value: string) => void
  categories: string[]
  activeCategory: string
  onCategoryChange: (value: string) => void
}

const SearchPill: React.FC<{
  value: string
  onChange: (value: string) => void
  onSubmit: () => void
}> = ({ value, onChange, onSubmit }) => {
  return (
    <div className="flex h-[58px] items-center gap-4 rounded-[80px] border border-[rgba(74,74,74,0.70)] bg-[#1A1A1A] px-2 pr-4">
      <div className="flex h-full w-full items-center gap-[10px] rounded-[80px] px-4">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0">
          <path d="M21.5308 20.4693L16.8368 15.7762C18.1973 14.1428 18.8757 12.0478 18.7309 9.92691C18.5861 7.80604 17.6293 5.82265 16.0593 4.38932C14.4894 2.95599 12.4274 2.18308 10.3021 2.23138C8.17687 2.27968 6.15205 3.14547 4.64888 4.64864C3.14571 6.15181 2.27993 8.17663 2.23163 10.3019C2.18333 12.4271 2.95623 14.4892 4.38956 16.0591C5.82289 17.629 7.80629 18.5859 9.92715 18.7307C12.048 18.8755 14.1431 18.1971 15.7765 16.8365L20.4696 21.5306C20.5393 21.6003 20.622 21.6556 20.713 21.6933C20.8041 21.731 20.9017 21.7504 21.0002 21.7504C21.0988 21.7504 21.1963 21.731 21.2874 21.6933C21.3784 21.6556 21.4612 21.6003 21.5308 21.5306C21.6005 21.4609 21.6558 21.3782 21.6935 21.2871C21.7312 21.1961 21.7506 21.0985 21.7506 21C21.7506 20.9014 21.7312 20.8038 21.6935 20.7128C21.6558 20.6218 21.6005 20.539 21.5308 20.4693ZM3.75021 10.5C3.75021 9.16495 4.14609 7.8599 4.88779 6.74987C5.62949 5.63984 6.6837 4.77467 7.9171 4.26378C9.1505 3.75289 10.5077 3.61922 11.8171 3.87967C13.1264 4.14012 14.3292 4.78299 15.2732 5.727C16.2172 6.671 16.8601 7.87374 17.1205 9.18311C17.381 10.4925 17.2473 11.8497 16.7364 13.0831C16.2255 14.3165 15.3603 15.3707 14.2503 16.1124C13.1403 16.8541 11.8352 17.25 10.5002 17.25C8.71061 17.248 6.99488 16.5362 5.72944 15.2708C4.464 14.0053 3.7522 12.2896 3.75021 10.5Z" fill="#9E9E9E" />
        </svg>
        <input
          type="text"
          value={value}
          onChange={(event) => onChange(event.target.value)}
          onKeyDown={(event) => {
            if (event.key === 'Enter') onSubmit()
          }}
          placeholder="Search"
          className="w-full bg-transparent border-none outline-none font-poppins text-[16px] font-medium leading-[26px] text-[#FCFCFC] placeholder:text-[#FCFCFC]"
        />
      </div>
    </div>
  )
}

const PartnershipsFilters: React.FC<PartnershipsFiltersProps> = ({
  searchValue,
  onSearchChange,
  onSearchSubmit,
  locations,
  verticals,
  paymentModels,
  locationValue,
  verticalValue,
  paymentModelValue,
  onLocationChange,
  onVerticalChange,
  onPaymentModelChange,
  categories,
  activeCategory,
  onCategoryChange,
}) => {
  return (
    <div className="flex w-full flex-col items-start gap-6">
      <div className="grid w-full grid-cols-1 gap-[10px] md:grid-cols-2 xl:flex xl:items-start xl:gap-[10px]">
        <div className="w-full xl:flex-1">
          <SearchPill value={searchValue} onChange={onSearchChange} onSubmit={onSearchSubmit} />
        </div>
        <div className="w-full xl:flex-1">
          <MobileCategorySelector
            value={locationValue}
            options={locations}
            onSelect={onLocationChange}
            buttonClassName="h-[58px] border-[rgba(74,74,74,0.70)] bg-[#1A1A1A] text-[#FCFCFC]"
            iconClassName="text-[#FCFCFC]"
          />
        </div>
        <div className="w-full xl:flex-1">
          <MobileCategorySelector
            value={verticalValue}
            options={verticals}
            onSelect={onVerticalChange}
            buttonClassName="h-[58px] border-[rgba(74,74,74,0.70)] bg-[#1A1A1A] text-[#FCFCFC]"
            iconClassName="text-[#FCFCFC]"
          />
        </div>
        <div className="w-full xl:flex-1">
          <MobileCategorySelector
            value={paymentModelValue}
            options={paymentModels}
            onSelect={onPaymentModelChange}
            buttonClassName="h-[58px] border-[rgba(74,74,74,0.70)] bg-[#1A1A1A] text-[#FCFCFC]"
            iconClassName="text-[#FCFCFC]"
          />
        </div>
        <button
          type="button"
          onClick={onSearchSubmit}
          className="h-[58px] w-full rounded-[80px] bg-[#F29F04] px-6 font-poppins text-[16px] font-medium leading-[26px] text-[#0D0D0D] transition-colors hover:bg-[#df9303] xl:w-[200px] xl:flex-none"
        >
          Search
        </button>
      </div>

      <div className="flex max-w-full items-start gap-4 overflow-x-auto rounded-[80px] border border-[rgba(74,74,74,0.70)] bg-[#1A1A1A] p-2 pr-4 no-scrollbar">
        {categories.map((category) => (
          <button
            key={category}
            type="button"
            onClick={() => onCategoryChange(category)}
            className={`whitespace-nowrap rounded-[80px] px-4 py-2 font-poppins text-[16px] leading-[26px] transition-all ${
              activeCategory === category
                ? 'bg-[#F29F04] font-medium text-[#070707]'
                : 'font-normal text-[#FCFCFC] hover:text-[#F29F04]'
            }`}
          >
            {category}
          </button>
        ))}
      </div>
    </div>
  )
}

export default PartnershipsFilters
